...
core::HUNLFlatSolveGraph make_scaling_graph() {
    constexpr std::uint32_t kRootActions = 4;
    constexpr std::uint32_t kChanceOutcomes = 2;
    constexpr std::uint32_t kOpponentActions = 4;
    constexpr std::uint32_t kReplyActions = 4;

    core::HUNLFlatSolveGraph graph;
    graph.root = 0;
    graph.max_depth = 4;
    graph.max_actions = static_cast<std::uint8_t>(std::max({kRootActions, kOpponentActions, kReplyActions}));
    ...
    graph.infosets.push_back(core::HUNLFlatInfoset{
        ...
        0,
        core::Street::Flop,
        static_cast<std::uint8_t>(kRootActions),
    });
    graph.infosets.push_back(core::HUNLFlatInfoset{
        opponent_infoset,
        1,
        kRootActions * kChanceOutcomes,
        {},
        1,
        1,
        core::Street::Flop,
        static_cast<std::uint8_t>(kOpponentActions),
    });
    graph.infosets.push_back(core::HUNLFlatInfoset{
        reply_infoset,
        1 + kRootActions * kChanceOutcomes,
        kRootActions * kChanceOutcomes * kOpponentActions,
        {},
        2,
        0,
        core::Street::Flop,
        static_cast<std::uint8_t>(kReplyActions),
    });
    graph.infoset_debug_keys = {"root-p0", "opp-p1", "reply-p0"};

    const std::uint32_t chance_begin = 1;
    const std::uint32_t opponent_begin = chance_begin + kRootActions;
    const std::uint32_t reply_begin = opponent_begin + kRootActions * kChanceOutcomes;
    const std::uint32_t terminal_begin = reply_begin + kRootActions * kChanceOutcomes * kOpponentActions;
    const std::uint32_t total_nodes =
        terminal_begin + kRootActions * kChanceOutcomes * kOpponentActions * kReplyActions;
    ...
    graph.children.reserve(
        kRootActions +
        kRootActions * kChanceOutcomes * kOpponentActions +
        kRootActions * kChanceOutcomes * kOpponentActions * kReplyActions);
    graph.chance_outcomes.reserve(kRootActions * kChanceOutcomes);
    graph.infoset_nodes.reserve(1U + kRootActions * kChanceOutcomes + kRootActions * kChanceOutcomes * kOpponentActions);

    graph.node_meta[0].child_begin = 0;
    graph.node_meta[0].child_count = kRootActions;
    ...
    graph.node_meta[0].action_count = static_cast<std::uint8_t>(kRootActions);
    graph.node_meta[0].has_infoset = true;
    graph.infoset_nodes.push_back(0);

    for (std::uint32_t root_action = 0; root_action < kRootActions; ++root_action) {
        ...
        graph.node_meta[chance_node].chance_count = kChanceOutcomes;
        ...
        for (std::uint32_t outcome = 0; outcome < kChanceOutcomes; ++outcome) {
            const auto opponent_index = root_action * kChanceOutcomes + outcome;
            ...
            graph.chance_outcomes.push_back(core::HUNLFlatChanceOutcome{
                ...
                1.0 / static_cast<double>(kChanceOutcomes),
                opponent_node,
                1U,
            });

            graph.node_meta[opponent_node].child_begin = static_cast<std::uint32_t>(graph.children.size());
            graph.node_meta[opponent_node].child_count = kOpponentActions;
            ...
            graph.node_meta[opponent_node].action_count = static_cast<std::uint8_t>(kOpponentActions);
            ...
            for (std::uint32_t opponent_action = 0; opponent_action < kOpponentActions; ++opponent_action) {
                const auto reply_index = opponent_index * kOpponentActions + opponent_action;
                ...
                graph.node_meta[reply_node].child_count = kReplyActions;
                ...
                graph.node_meta[reply_node].action_count = static_cast<std::uint8_t>(kReplyActions);
                graph.node_meta[reply_node].has_infoset = true;
                graph.node_depths[reply_node] = 3;
                graph.infoset_nodes.push_back(reply_node);

                for (std::uint32_t reply_action = 0; reply_action < kReplyActions; ++reply_action) {
                    const auto terminal_index = reply_index * kReplyActions + reply_action;
    ...
    graph.depth_slices = {
        ...
        core::HUNLFlatSlice{1, kRootActions},
        core::HUNLFlatSlice{opponent_begin, kRootActions * kChanceOutcomes},
        core::HUNLFlatSlice{reply_begin, kRootActions * kChanceOutcomes * kOpponentActions},
        core::HUNLFlatSlice{
            terminal_begin,
            kRootActions * kChanceOutcomes * kOpponentActions * kReplyActions,
        },
...
void print_results(const BenchmarkConfig& config, const std::vector<BenchmarkResult>& results) {
    ...
    std::cout << "  graph=synthetic-easy-flop"
