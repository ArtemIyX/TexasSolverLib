...
texas::MultiwaySolverLimits bounded_limits() {
...
texas::MultiwayMemoryBudget roomy_budget() {
    return {800'000'000U, 1'000'000'000U, 900'000'000U};
}

texas::MultiwayMemoryPreflight baseline(texas::MultiwayMemoryInputs inputs = {}) {
    return texas::preflight_multiway_memory(bounded_limits(), roomy_budget(), inputs);
}

std::uint64_t root_bytes(const texas::MultiwayMemoryEstimate& estimate) {
...
std::uint64_t row_bytes(const texas::MultiwayMemoryEstimate& estimate) {
...
TEST_CASE(multiway_p7_memory_default_warning_is_48_gib) {
    const texas::MultiwayMemoryBudget budget;
...
TEST_CASE(multiway_p7_memory_roomy_preflight_admits_mandatory_stages) {
    ...
    EXPECT_EQ(result.status, texas::MultiwayMemoryStatus::Ok);
    EXPECT_EQ(result.highest_admitted_stage, texas::MultiwayMemoryAdmissionStage::WorkerDeltas);
}
...
TEST_CASE(multiway_p7_memory_sparse_row_metadata_is_accounted) {
    ...
    EXPECT_EQ(
        ...
        3U * static_cast<std::uint64_t>(sizeof(texas::MultiwaySparseRowMetadata)));
}
...
TEST_CASE(multiway_p7_memory_range_rows_account_for_vector_owners) {
    ...
    EXPECT_EQ(
        ...
        6U * static_cast<std::uint64_t>(sizeof(std::vector<texas::MultiwayWeightedHole>)));
}
...
TEST_CASE(multiway_p7_memory_range_entries_account_for_source_and_compiled_copies) {
    ...
    EXPECT_EQ(
        baseline(inputs).estimate.range_row_bytes,
        7U * 2U * static_cast<std::uint64_t>(sizeof(texas::MultiwayWeightedHole)));
}
...
TEST_CASE(multiway_p7_memory_off_tree_menu_entries_use_action_descriptor_size) {
    ...
    EXPECT_EQ(
        ...
        9U * static_cast<std::uint64_t>(sizeof(texas::MultiwayActionDescriptor)));
}
...
TEST_CASE(multiway_p7_memory_export_accounts_for_two_probability_buffers) {
    ...
    EXPECT_EQ(
        ...
        3U * 2U * static_cast<std::uint64_t>(sizeof(texas::MultiwayRootActionProbability)));
}
...
TEST_CASE(multiway_p7_memory_optional_cache_degrades_above_operating_cap) {
    ...
    const auto result = texas::preflight_multiway_memory(
        ...
        inputs);
    EXPECT_EQ(result.status, texas::MultiwayMemoryStatus::Warning);
    EXPECT_TRUE(result.degraded);
}
...
TEST_CASE(multiway_p7_memory_root_above_operating_cap_is_rejected_before_admission) {
    ...
    EXPECT_EQ(result.status, texas::MultiwayMemoryStatus::Rejected);
    EXPECT_EQ(result.highest_admitted_stage, texas::MultiwayMemoryAdmissionStage::None);
}
...
TEST_CASE(multiway_p7_memory_root_at_operating_cap_is_admitted) {
    ...
    EXPECT_EQ(result.highest_admitted_stage, texas::MultiwayMemoryAdmissionStage::Root);
}
...
TEST_CASE(multiway_p7_memory_worker_deltas_above_operating_cap_stop_after_rows) {
    ...
    EXPECT_EQ(result.status, texas::MultiwayMemoryStatus::Rejected);
    EXPECT_EQ(result.highest_admitted_stage, texas::MultiwayMemoryAdmissionStage::SparseRows);
...
TEST_CASE(multiway_p7_memory_public_state_estimate_saturates_on_overflow) {
    ...
    const auto result = texas::preflight_multiway_memory(limits);
