...
TEST_CASE(multiway_traversal_appends_external_sampling_worker_deltas) {
    texas::MultiwayWorkerDeltaStream stream(0, 4);
    ...
    EXPECT_TRUE(texas::MultiwayExternalSamplingTraversal::append_infoset_update(
...
TEST_CASE(multiway_traversal_rejects_an_insufficient_delta_stream) {
    texas::MultiwayWorkerDeltaStream stream(0, 1);
    texas::MultiwayExternalSamplingRequest request;
