...
bool contains_card(const core::CanonicalComboCards& hole, std::uint8_t card) {
...
TEST_CASE(canonical_combos_exhaustively_round_trip_all_hunl_holes) {
    ...
    std::array<bool, core::CANONICAL_HOLE_COMBINATION_COUNT> seen = {};

    for (std::size_t raw_id = 0U; raw_id < core::CANONICAL_HOLE_COMBINATION_COUNT; ++raw_id) {
        const auto id = static_cast<core::CanonicalComboId>(raw_id);
        const auto hole = combos.cards(id);
        EXPECT_TRUE(core::is_hunl_card(hole[0]));
        EXPECT_TRUE(core::is_hunl_card(hole[1]));
    ...
    const core::CanonicalComboCards first = {8U, 9U};
    const core::CanonicalComboCards last = {58U, 59U};
    EXPECT_EQ(combos.cards(0U), first);
    EXPECT_EQ(combos.cards(static_cast<core::CanonicalComboId>(
                  core::CANONICAL_HOLE_COMBINATION_COUNT - 1U)),
              last);
    EXPECT_THROW(combos.cards(static_cast<core::CanonicalComboId>(
...
TEST_CASE(canonical_combos_reject_invalid_cards_and_dead_card_lists) {
    const auto& combos = core::canonical_combos();
    EXPECT_THROW(combos.id({0U, 8U}), std::invalid_argument);
    EXPECT_THROW(combos.id({7U, 8U}), std::invalid_argument);
    EXPECT_THROW(combos.id({8U, 8U}), std::invalid_argument);
    EXPECT_THROW(combos.id({8U, 60U}), std::invalid_argument);
    EXPECT_THROW(combos.id({8U, 255U}), std::invalid_argument);

    const std::array<std::uint8_t, 2> duplicate_dead = {8U, 8U};
    const std::array<std::uint8_t, 1> invalid_dead = {0U};
...
TEST_CASE(canonical_combo_legal_masks_match_exhaustive_blocker_rules) {
    ...
    EXPECT_EQ(no_dead.count(), core::CANONICAL_HOLE_COMBINATION_COUNT);
    ...
    const std::array<std::uint8_t, 5> river = {8U, 9U, 10U, 11U, 12U};
    const auto river_mask = combos.legal_mask(river.data(), river.size());
    EXPECT_EQ(river_mask.count(), std::size_t{1081U});

    for (std::size_t raw_id = 0U; raw_id < core::CANONICAL_HOLE_COMBINATION_COUNT; ++raw_id) {
        const auto hole = combos.cards(static_cast<core::CanonicalComboId>(raw_id));
...
TEST_CASE(canonical_combo_range_enumeration_and_masks_preserve_canonical_order) {
    const std::vector<std::uint8_t> board = {8U, 33U, 58U};
    const auto index = core::enumerate_combos(board);
    const auto& combos = core::canonical_combos();
    ...
    const auto mask = core::dead_card_mask(index, {dealt});
