#include "core/canonical_combo.hpp"
#include "test_harness.hpp"

#include <algorithm>
#include <array>
#include <cstddef>
#include <cstdint>
#include <stdexcept>
#include <vector>

namespace {

bool contains_card(const texas::core::CanonicalComboCards& hole, std::uint8_t card) {
    return hole[0] == card || hole[1] == card;
}

}  // namespace

TEST_CASE(canonical_combos_exhaustively_round_trip_all_hunl_holes) {
    const auto& combos = texas::core::canonical_combos();
    std::array<bool, texas::core::CANONICAL_HOLE_COMBINATION_COUNT> seen = {};

    for (std::size_t raw_id = 0U; raw_id < texas::core::CANONICAL_HOLE_COMBINATION_COUNT; ++raw_id) {
        const auto id = static_cast<texas::core::CanonicalComboId>(raw_id);
        const auto hole = combos.cards(id);
        EXPECT_TRUE(texas::core::is_card_index(hole[0]));
        EXPECT_TRUE(texas::core::is_card_index(hole[1]));
        EXPECT_TRUE(hole[0] < hole[1]);
        EXPECT_EQ(combos.id(hole), id);
        EXPECT_EQ(combos.id({hole[1], hole[0]}), id);
        EXPECT_TRUE(!seen[id]);
        seen[id] = true;
    }

    for (const auto was_seen : seen) EXPECT_TRUE(was_seen);

    const texas::core::CanonicalComboCards first = {0U, 1U};
    const texas::core::CanonicalComboCards last = {50U, 51U};
    EXPECT_EQ(combos.cards(0U), first);
    EXPECT_EQ(combos.cards(static_cast<texas::core::CanonicalComboId>(
                  texas::core::CANONICAL_HOLE_COMBINATION_COUNT - 1U)),
              last);
    EXPECT_THROW(combos.cards(static_cast<texas::core::CanonicalComboId>(
                     texas::core::CANONICAL_HOLE_COMBINATION_COUNT)),
                 std::out_of_range);
}

TEST_CASE(canonical_combos_reject_invalid_cards_and_dead_card_lists) {
    const auto& combos = texas::core::canonical_combos();
    EXPECT_THROW(combos.id({52U, 8U}), std::invalid_argument);
    EXPECT_THROW(combos.id({7U, 52U}), std::invalid_argument);
    EXPECT_THROW(combos.id({0U, 0U}), std::invalid_argument);
    EXPECT_THROW(combos.id({0U, 52U}), std::invalid_argument);
    EXPECT_THROW(combos.id({0U, 255U}), std::invalid_argument);

    const std::array<std::uint8_t, 2> duplicate_dead = {0U, 0U};
    const std::array<std::uint8_t, 1> invalid_dead = {52U};
    EXPECT_THROW(combos.legal_mask(nullptr, 1U), std::invalid_argument);
    EXPECT_THROW(combos.legal_mask(duplicate_dead.data(), duplicate_dead.size()), std::invalid_argument);
    EXPECT_THROW(combos.legal_mask(invalid_dead.data(), invalid_dead.size()), std::invalid_argument);
}

TEST_CASE(canonical_combo_legal_masks_match_exhaustive_blocker_rules) {
    const auto& combos = texas::core::canonical_combos();
    const auto no_dead = combos.legal_mask(nullptr, 0U);
    EXPECT_EQ(no_dead.count(), texas::core::CANONICAL_HOLE_COMBINATION_COUNT);

    const std::array<std::uint8_t, 3> flop = {0U, 25U, 50U};
    const auto flop_mask = combos.legal_mask(flop.data(), flop.size());
    EXPECT_EQ(flop_mask.count(), std::size_t{1176U});

    const std::array<std::uint8_t, 5> river = {0U, 1U, 2U, 3U, 4U};
    const auto river_mask = combos.legal_mask(river.data(), river.size());
    EXPECT_EQ(river_mask.count(), std::size_t{1081U});

    for (std::size_t raw_id = 0U; raw_id < texas::core::CANONICAL_HOLE_COMBINATION_COUNT; ++raw_id) {
        const auto hole = combos.cards(static_cast<texas::core::CanonicalComboId>(raw_id));
        const auto blocked = std::find(flop.begin(), flop.end(), hole[0]) != flop.end() ||
            std::find(flop.begin(), flop.end(), hole[1]) != flop.end();
        EXPECT_EQ(flop_mask.test(raw_id), !blocked);
    }
}

TEST_CASE(canonical_combo_range_enumeration_and_masks_preserve_canonical_order) {
    const std::vector<std::uint8_t> board = {0U, 25U, 50U};
    const auto& combos = texas::core::canonical_combos();
    const auto board_mask = combos.legal_mask(board.data(), board.size());

    EXPECT_EQ(board_mask.count(), std::size_t{1176U});
    for (std::size_t raw_id = 0U;
         raw_id < texas::core::CANONICAL_HOLE_COMBINATION_COUNT;
         ++raw_id) {
        const auto hole = combos.cards(static_cast<texas::core::CanonicalComboId>(raw_id));
        const bool blocked = contains_card(hole, board[0]) ||
            contains_card(hole, board[1]) || contains_card(hole, board[2]);
        EXPECT_EQ(board_mask.test(raw_id), !blocked);
    }

    const std::uint8_t dealt = 9U;
    const std::vector<std::uint8_t> dead_cards = {board[0], board[1], board[2], dealt};
    const auto dealt_mask = combos.legal_mask(dead_cards.data(), dead_cards.size());
    std::size_t newly_blocked = 0U;
    for (std::size_t raw_id = 0U;
         raw_id < texas::core::CANONICAL_HOLE_COMBINATION_COUNT;
         ++raw_id) {
        const auto hole = combos.cards(static_cast<texas::core::CanonicalComboId>(raw_id));
        const bool expected = board_mask.test(raw_id) && !contains_card(hole, dealt);
        EXPECT_EQ(dealt_mask.test(raw_id), expected);
        if (board_mask.test(raw_id) && !dealt_mask.test(raw_id)) ++newly_blocked;
    }
    EXPECT_EQ(newly_blocked, std::size_t{48U});
}
