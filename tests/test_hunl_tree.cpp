...
TEST_CASE(hunl_tree_builds_river_subgame_with_terminals) {
    ...
    for (const auto& node : tree.nodes) {
        fold_seen = fold_seen || node.terminal_kind.tag == texas::TerminalKindTag::Fold;
        showdown_seen = showdown_seen || node.terminal_kind.tag == texas::TerminalKindTag::Showdown;
    }
...
TEST_CASE(hunl_tree_depth_limit_marks_cutoff_nodes_as_distinct_leaves) {
    ...
    const auto tree = texas::HUNLTree::build(std::make_shared<const texas::HUNLConfig>(config));

    bool depth_limited_seen = false;
    for (const auto& node : tree.nodes) {
        depth_limited_seen = depth_limited_seen || node.depth_limited_leaf;
        if (node.depth_limited_leaf) {
            EXPECT_EQ(node.terminal_kind.tag, texas::TerminalKindTag::NonTerminal);
