...
TEST_CASE(multiway_training_config_binds_default_rules_and_artifacts) {
    core::MultiwayBlueprintTrainingConfig config;
    config.validate();
...
TEST_CASE(multiway_training_schedule_controls_linear_weighting_and_pruning) {
    core::MultiwayBlueprintIterationSchedule schedule;
...
TEST_CASE(multiway_checkpoint_resume_identity_rejects_different_artifacts) {
    ...
    auto snapshot = core::MultiwayBlueprintSnapshot{};
    ...
    snapshot.actions = {{{core::MultiwayAction::Check, 0U, 0, 11U}, 65535U}};

    core::MultiwayCheckpoint::validate_resume_identity(snapshot, expected);
    ++config.blueprint.bucket_model_version;
    EXPECT_THROW(
        core::MultiwayCheckpoint::validate_resume_identity(snapshot, config.identity()),
        std::invalid_argument);
}

TEST_CASE(multiway_training_metadata_persists_policy_variant_contract) {
    core::MultiwayBlueprintTrainingMetadata metadata;
    ...
    snapshot.policy_kind = core::MultiwayBlueprintPolicyKind::LateWindowAverage;
    snapshot.training = metadata;
    snapshot.actions = {{{core::MultiwayAction::Check, 0U, 0, 5U}, 65535U}};
    snapshot.validate();
    EXPECT_EQ(snapshot.policy_kind, core::MultiwayBlueprintPolicyKind::LateWindowAverage);
}
