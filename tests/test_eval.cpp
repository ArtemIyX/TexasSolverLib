...
TEST_CASE(eval_five_card_ordering_and_bridge) {
    ...
    const auto s_high = texas::evaluate_n(std::vector<std::uint8_t>(high_card.begin(), high_card.end()));
    const auto s_pair = texas::evaluate_n(std::vector<std::uint8_t>(pair.begin(), pair.end()));
    const auto s_sf = texas::evaluate_n(std::vector<std::uint8_t>(straight_flush.begin(), straight_flush.end()));

    EXPECT_TRUE(s_high < s_pair);
    EXPECT_TRUE(s_pair < s_sf);
    EXPECT_EQ(s_high, texas::Strength::evaluate_5(high_card));
    EXPECT_EQ(s_pair, texas::Strength::evaluate_5(pair));
    EXPECT_EQ(s_sf, texas::Strength::evaluate_5(straight_flush));
}
...
TEST_CASE(eval_six_card_uses_fast_path) {
    ...
    const auto s_strong = texas::evaluate_n(std::vector<std::uint8_t>(strong.begin(), strong.end()));
    const auto s_weak = texas::evaluate_n(std::vector<std::uint8_t>(weak.begin(), weak.end()));
...
TEST_CASE(eval_seven_card_uses_fast_path) {
    ...
    const auto s_n = texas::evaluate_n(std::vector<std::uint8_t>(seven.begin(), seven.end()));
...
TEST_CASE(eval_eight_card_falls_back) {
    ...
    const auto s8 = texas::evaluate_n(eight);
    const auto s7 = texas::Strength::evaluate_7({c(14, 0), c(14, 1), c(13, 0), c(12, 1), c(11, 2), c(10, 3), c(9, 0)});
