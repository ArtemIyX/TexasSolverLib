...
TEST_CASE(hunl_bucket_model_bucket_counts_match_artifact_metadata) {
    ...
    const auto path = test_support::write_abstraction_fixture(
        ...
        test_support::AbstractionFixtureOptions{{1, 1, 5}, texas::ABSTRACTION_SCHEMA_VERSION, "custom-river-5", std::nullopt});
...
TEST_CASE(hunl_bucket_model_every_valid_hand_maps_to_a_bucket) {
    ...
    const auto path = test_support::write_abstraction_fixture(
        ...
        test_support::AbstractionFixtureOptions{{1, 1, 4}, texas::ABSTRACTION_SCHEMA_VERSION, "river-4", std::nullopt});
...
TEST_CASE(hunl_bucket_model_invalid_board_or_hand_combinations_fail_cleanly) {
    ...
    EXPECT_THROW(texas::lookup_bucket(tables, invalid_board, other_hole, texas::Street::River), std::out_of_range);
    EXPECT_THROW(texas::lookup_bucket(tables, config->initial_board, invalid_hole, texas::Street::River), std::out_of_range);
...
TEST_CASE(hunl_bucket_model_dense_bucket_ids_are_documented_by_runtime_map) {
    ...
    const auto path = test_support::write_abstraction_fixture(
        ...
        test_support::AbstractionFixtureOptions{{1, 1, 4}, texas::ABSTRACTION_SCHEMA_VERSION, "sparse-assignment", std::nullopt});
...
TEST_CASE(hunl_bucket_model_custom_bucket_schema_round_trips_through_load_and_lookup) {
    ...
    const auto path = test_support::write_abstraction_fixture(
        ...
        std::nullopt,
        config->initial_board,
        [](texas::Street, std::size_t index, const std::array<std::uint8_t, 2>& hole) {
            return static_cast<std::uint8_t>((index + hole[0] + hole[1]) % 6U);
        },
        test_support::AbstractionFixtureOptions{{1, 1, 6}, texas::ABSTRACTION_SCHEMA_VERSION, "river-6-schema", std::nullopt});
    ...
    EXPECT_EQ(
        texas::lookup_bucket(tables, config->initial_board, live_hands[0], texas::Street::River),
...
TEST_CASE(hunl_bucket_model_set_bucket_weights_normalizes_custom_weights) {
    ...
    const auto graph = texas::HUNLFlatSolveGraph::build(config);
    auto map = texas::HUNLFlatBucketMap::from_abstraction(graph, texas::load_abstraction(path));
...
TEST_CASE(hunl_bucket_terminal_cache_reuses_same_board_stats_across_nodes) {
    ...
    const auto graph = texas::HUNLFlatSolveGraph::build(config);
    const auto map = texas::HUNLFlatBucketMap::from_abstraction(graph, texas::load_abstraction(path));
    const auto table = texas::HUNLBucketTerminalTable::build(graph, map);
    ...
    EXPECT_TRUE(table.estimated_bytes() > sizeof(texas::HUNLBucketTerminalTable));
