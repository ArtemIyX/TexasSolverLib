...
core::MultiwayWorkerDelta worker_delta(
    std::int32_t seat = 0,
...
std::array<core::MultiwayWorkerBatch, 8U> partition(
    std::uint64_t trajectories,
    std::size_t workers,
    std::size_t& count) {
    std::array<core::MultiwayWorkerBatch, 8U> batches{};
    count = core::MultiwayScheduler::partition_deterministic_into(
...
TEST_CASE(multiway_p7_hot_path_empty_vector_partition_has_one_batch) {
    const auto batches = core::MultiwayScheduler::partition_deterministic(0U, 4U);
...
TEST_CASE(multiway_p7_hot_path_workers_are_clamped_to_output_capacity) {
    std::array<core::MultiwayWorkerBatch, 2U> batches{};
    const auto count = core::MultiwayScheduler::partition_deterministic_into(
...
TEST_CASE(multiway_p7_hot_path_null_partition_output_is_rejected) {
    EXPECT_EQ(core::MultiwayScheduler::partition_deterministic_into(3U, 2U, nullptr, 2U), 0U);
}

TEST_CASE(multiway_p7_hot_path_zero_partition_capacity_is_rejected) {
    core::MultiwayWorkerBatch batch{};
    EXPECT_EQ(core::MultiwayScheduler::partition_deterministic_into(3U, 2U, &batch, 0U), 0U);
}
...
TEST_CASE(multiway_p7_hot_path_vector_partition_matches_caller_owned_partition) {
    ...
    const auto owned = core::MultiwayScheduler::partition_deterministic(13U, 4U);
    for (std::size_t index = 0U; index < count; ++index) {
...
TEST_CASE(multiway_p7_hot_path_partition_does_not_touch_unused_slots) {
    std::array<core::MultiwayWorkerBatch, 4U> batches{};
    batches[3].worker_index = 99U;
    (void)core::MultiwayScheduler::partition_deterministic_into(2U, 4U, batches.data(), batches.size());
    EXPECT_EQ(batches[3].worker_index, 99U);
}

TEST_CASE(multiway_p7_hot_path_partition_is_repeatable_in_reused_storage) {
    std::array<core::MultiwayWorkerBatch, 4U> batches{};
    (void)core::MultiwayScheduler::partition_deterministic_into(17U, 4U, batches.data(), batches.size());
...
TEST_CASE(multiway_p7_hot_path_delta_stream_exposes_fixed_capacity_metadata) {
    const core::MultiwayWorkerDeltaStream stream(7U, 12U);
...
TEST_CASE(multiway_p7_hot_path_zero_capacity_stream_rejects_append) {
    core::MultiwayWorkerDeltaStream stream(0U, 0U);
...
TEST_CASE(multiway_p7_hot_path_stream_rewind_truncates_entries) {
    core::MultiwayWorkerDeltaStream stream(0U, 3U);
