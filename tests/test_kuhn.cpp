...
TEST_CASE(kuhn_initial_state_is_chance_node) {
    const auto state = texas::KuhnState::initial();
...
TEST_CASE(kuhn_showdown_and_fold_utilities_match_rust_tests) {
    const auto showdown = texas::KuhnState::initial().next_state(13).next_state(12).next_state(texas::PASS).next_state(texas::PASS);
    EXPECT_TRUE(showdown.is_terminal());
    EXPECT_EQ(showdown.utility()[0], 1.0);
    EXPECT_EQ(showdown.utility()[1], -1.0);

    const auto fold = texas::KuhnState::initial().next_state(11).next_state(13).next_state(texas::PASS).next_state(texas::BET).next_state(texas::PASS);
...
TEST_CASE(kuhn_infoset_key_includes_private_card_and_history) {
    const auto state = texas::KuhnState::initial().next_state(12).next_state(13).next_state(texas::PASS).next_state(texas::BET);
