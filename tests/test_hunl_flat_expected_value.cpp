...
texas::HUNLFlatSolveGraph make_shared_infoset_chance_graph() {
...
TEST_CASE(hunl_flat_expected_value_averages_bucketed_export_rows_without_dynamic_state_walks) {
    ...
    const auto strategy_table = texas::build_flat_average_strategy_table(graph, strategy);
    const auto terminal_values = texas::build_flat_terminal_value_table(graph);
    const auto value = texas::compute_flat_expected_value(graph, strategy_table.view(), &terminal_values);
...
TEST_CASE(hunl_flat_dcfr_exports_infoset_indexed_average_strategy_table) {
    ...
    EXPECT_TRUE(table.layout == texas::HUNLFlatValueLayout::InfosetHandAction);
...
TEST_CASE(hunl_flat_expected_value_benchmark_fast_path_matches_full_flat_value) {
    ...
    const auto full_value = texas::compute_flat_expected_value(graph, strategy_table.view(), &terminal_values);
    const auto fast_value =
        texas::compute_flat_expected_value_p0_benchmark(graph, strategy_table.view(), terminal_values_p0);
