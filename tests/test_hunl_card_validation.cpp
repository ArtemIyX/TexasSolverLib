...
TEST_CASE(hunl_range_validation_rejects_invalid_cards_and_nonfinite_or_zero_mass_weights) {
    ...
    auto invalid_card = valid_config();
    ...
    auto board_blocked = valid_config();
    board_blocked.player_ranges[1] = single_hand_range(board_blocked.initial_board[0], valid_hole[1], 1.0);
    EXPECT_THROW(board_blocked.validate(), std::invalid_argument);

    auto duplicate_hand = valid_config();
    ...
    negative.player_ranges[0] = single_hand_range(valid_hole[0], valid_hole[1], -1.0);
    EXPECT_THROW(negative.validate(), std::invalid_argument);

    auto nan_weight = valid_config();
    ...
    auto invalid_bucket_weight = valid_config();
    invalid_bucket_weight.flat_solve_mode = core::HUNLFlatSolveMode::Bucketed;
    core::HUNLRangeInput bucket_range;
    bucket_range.bucket_weights.push_back({core::Street::Turn, 0U, std::numeric_limits<double>::infinity()});
    invalid_bucket_weight.player_ranges[0] = bucket_range;
    EXPECT_THROW(invalid_bucket_weight.validate(), std::invalid_argument);

    auto zero_bucket_mass = valid_config();
    zero_bucket_mass.flat_solve_mode = core::HUNLFlatSolveMode::Bucketed;
    core::HUNLRangeInput zero_bucket_range;
    zero_bucket_range.bucket_weights.push_back({core::Street::Turn, 0U, 0.0});
    zero_bucket_mass.player_ranges[0] = zero_bucket_range;
