...
TEST_CASE(br_walk_mode_contract_rejects_vector_and_unknown_modes_before_tree_construction) {
    ...
    for (std::uint8_t raw_mode = 1; raw_mode <= 20; ++raw_mode) {
        EXPECT_THROW(
            core::compute_exploitability_and_value_with_mode(
                config, strategy, static_cast<core::BrWalkMode>(raw_mode)),
...
TEST_CASE(br_walk_mode_contract_keeps_per_combo_available) {
    const auto config = core::default_tiny_subgame();
    const std::unordered_map<std::string, std::vector<double>> strategy;
    const auto output = core::compute_exploitability_and_value_with_mode(
        config, strategy, core::BrWalkMode::PerCombo);
