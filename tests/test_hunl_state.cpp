...
texas::HUNLState flop_state(int starting_stack, int pot) {
    texas::HUNLConfig cfg;
    cfg.starting_stack = starting_stack;
    cfg.starting_street = texas::Street::Flop;
    cfg.initial_board = {
...
TEST_CASE(hunl_initial_postflop_invariants) {
    ...
    EXPECT_EQ(state.street, texas::Street::River);
...
TEST_CASE(hunl_river_root_actions_match_rust_test) {
    ...
    EXPECT_TRUE(std::find(actions.begin(), actions.end(), texas::ACTION_CHECK) != actions.end());
    EXPECT_TRUE(std::find(actions.begin(), actions.end(), texas::ACTION_ALL_IN) != actions.end());
    EXPECT_TRUE(std::find(actions.begin(), actions.end(), texas::ACTION_FOLD) == actions.end());
    EXPECT_TRUE(std::find(actions.begin(), actions.end(), texas::ACTION_CALL) == actions.end());
}

TEST_CASE(hunl_facing_bet_has_fold_and_call_only_here) {
    const auto after_bet = river_state().apply(texas::ACTION_ALL_IN);
...
TEST_CASE(hunl_fold_and_showdown_utilities_match_reference) {
    const auto folded =
        river_state().apply(texas::ACTION_ALL_IN).apply(texas::ACTION_FOLD);
    EXPECT_TRUE(folded.is_terminal());
    EXPECT_NEAR(folded.utility()[0], 0.0, 1e-9);
    EXPECT_NEAR(folded.utility()[1], 10.0, 1e-9);

    const auto showdown = river_state().apply(texas::ACTION_CHECK).apply(texas::ACTION_CHECK);
    EXPECT_TRUE(showdown.is_terminal());
    EXPECT_EQ(showdown.street, texas::Street::Showdown);
...
TEST_CASE(hunl_apply_rejects_semantically_illegal_known_player_actions) {
    ...
    EXPECT_THROW(root.apply(texas::ACTION_FOLD), std::invalid_argument);
    EXPECT_THROW(root.apply(texas::ACTION_CALL), std::invalid_argument);
    EXPECT_THROW(root.apply(texas::ACTION_RAISE_33), std::invalid_argument);

    const auto facing_bet = root.apply(texas::ACTION_ALL_IN);
    EXPECT_THROW(facing_bet.apply(texas::ACTION_CHECK), std::invalid_argument);
    EXPECT_THROW(facing_bet.apply(texas::ACTION_BET_33), std::invalid_argument);

    const auto terminal = root.apply(texas::ACTION_CHECK).apply(texas::ACTION_CHECK);
    EXPECT_THROW(terminal.apply(texas::ACTION_CHECK), std::invalid_argument);
}
...
TEST_CASE(hunl_apply_rejects_twenty_invalid_or_blocked_chance_actions) {
    ...
    const std::array<int, 20> invalid = {
        ...
        texas::card_to_int(13, 3),
        texas::card_to_int(12, 2),
        texas::card_to_int(12, 1),
        60, 61, 62, 63,
    };
...
TEST_CASE(hunl_infoset_key_preserves_cards_and_history) {
    ...
    const auto after_bet =
        state.apply(texas::ACTION_CHECK).apply(texas::ACTION_BET_75);
    EXPECT_EQ(
...
TEST_CASE(hunl_infoset_validator_rejects_more_than_twenty_malformed_fixed_encodings) {
    ...
    std::vector<texas::HUNLInfosetEncoding> invalid;
    ...
    for (const auto& encoding : invalid) {
        EXPECT_THROW(
            texas::validate_hunl_infoset_encoding(encoding),
            std::invalid_argument);
        EXPECT_THROW(texas::hunl_infoset_key(encoding), std::invalid_argument);
    }
}

TEST_CASE(hunl_infoset_history_uses_exact_capacity_and_rejects_truncation) {
    auto exact = river_state();
    exact.current_street_history_codes.assign(texas::HUNL_MAX_HISTORY_CODES, 2);
    const auto encoding = exact.infoset_encoding(0);
    EXPECT_EQ(encoding.history_count, texas::HUNL_MAX_HISTORY_CODES);
    EXPECT_EQ(encoding.street_lengths[0], texas::HUNL_MAX_HISTORY_CODES);
...
TEST_CASE(hunl_infoset_history_keeps_large_opening_bets_distinct_from_raises) {
    ...
    const auto state = texas::HUNLState::initial(
        std::make_shared<const texas::HUNLConfig>(config));
    const auto facing_bet = state.apply(texas::ACTION_CHECK)
                                .apply(texas::ACTION_BET_100);
    EXPECT_EQ(
...
TEST_CASE(hunl_flat_backend_populates_value_and_exploitability) {
    ...
    EXPECT_EQ(output.quality_metric, texas::HUNLQualityMetric::PerPlayerExploitability);
...
TEST_CASE(hunl_public_postflop_solver_fails_closed_for_depth_limit_without_shared_leaf_evaluator) {
    ...
    EXPECT_THROW(texas::solve_hunl_postflop(config, 1, 1.5, 0.0, 2.0), std::invalid_argument);
}

TEST_CASE(hunl_postflop_raise_cap_blocks_further_raises) {
    const auto state = flop_state(100000, 200);
    const auto capped = state.apply(texas::ACTION_CHECK)
                            .apply(texas::ACTION_BET_100)
                            .apply(texas::ACTION_RAISE_33)
                            .apply(texas::ACTION_RAISE_33);
