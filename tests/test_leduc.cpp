...
TEST_CASE(leduc_initial_state_matches_rust_port) {
    const auto state = texas::LeducState::initial();
...
TEST_CASE(leduc_infoset_key_and_fold_utility) {
    ...
    state = state.next_state(texas::LEDUC_RAISE);
    state = state.next_state(texas::LEDUC_FOLD);
