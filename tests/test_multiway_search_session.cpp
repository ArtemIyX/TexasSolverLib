...
constexpr std::uint8_t card(std::uint8_t rank, std::uint8_t suit) {
    return texas::core::card_to_int(rank, suit);
}
        ...
        for (std::uint8_t second = static_cast<std::uint8_t>(first + 1U); second < 52U; ++second) {
            for (const auto board_card : compact_board) {
                if (first == board_card || second == board_card) {
                    assignments[texas::solver::multiway::MultiwayBucketTable::hole_index({first, second})] =
                        texas::solver::multiway::MULTIWAY_INVALID_BUCKET;
                }
            }
        }
    }
...
texas::solver::multiway::MultiwayRootSnapshot make_root(texas::core::Street street) {
    texas::games::multiway::MultiwayGameConfig game;
    ...
    game.big_blind = 100;
    game.street = street;
    const auto betting = texas::games::multiway::MultiwayState::initial(game).snapshot();

    const texas::solver::multiway::MultiwayActionAbstraction abstraction;
    texas::solver::multiway::MultiwayRootSnapshot root;
    std::vector<std::uint8_t> board;
    if (street != texas::core::Street::Preflop) {
        board = {card(2U, 0U), card(7U, 1U), card(9U, 2U)};
        if (street == texas::core::Street::Turn || street == texas::core::Street::River) board.push_back(card(11U, 3U));
        if (street == texas::core::Street::River) board.push_back(card(13U, 1U));
    }
    root.public_state = texas::solver::multiway::MultiwayPublicBuilder::make_root(
...
texas::solver::multiway::MultiwaySolverLimits limits() {
    texas::solver::multiway::MultiwaySolverLimits result;
...
texas::solver::multiway::MultiwaySolveRequest make_request(texas::solver::multiway::MultiwayRootSnapshot root) {
    texas::solver::multiway::MultiwayCFRConfig cfr;
    cfr.player_count = 2U;
    return texas::solver::multiway::MultiwaySolveRequest(std::move(root), cfr, limits());
}

texas::solver::multiway::MultiwayBucketRegistry make_buckets(const texas::solver::multiway::MultiwayRootSnapshot& root) {
    ...
    return texas::solver::multiway::MultiwayBucketRegistry({texas::solver::multiway::MultiwayBucketTable(
        texas::solver::multiway::make_multiway_model_identity(config), root.public_state.betting.street,
        compact_board, 1U, bucket_assignments(compact_board))});
}

texas::core::CanonicalComboId combo_id(texas::core::CanonicalComboCards cards) {
    return texas::core::canonical_combos().id(cards);
}

texas::core::Value session_leaf(
    const texas::solver::multiway::MultiwayLeafEvaluationRequest& leaf,
    const void*) noexcept {
    return static_cast<texas::core::Value>(
        leaf.betting->contributions[static_cast<std::size_t>(leaf.traverser)]);
}

texas::solver::multiway::MultiwaySearchSessionCleanSnapshot capture_clean_snapshot(
    const texas::solver::multiway::MultiwaySolveRequest& request,
    const texas::solver::multiway::MultiwayBucketRegistry& buckets,
    std::uint64_t seed) {
    texas::solver::multiway::MultiwaySearchSession session(request, {&buckets}, 1U);
    const texas::solver::multiway::MultiwayActionAbstraction abstraction;
    const texas::solver::multiway::MultiwayLeafEvaluator evaluator = {session_leaf, nullptr};
    texas::solver::multiway::MultiwayRootExternalSamplingTraversal traversal(
        session.coordinator(), session.coordinator().root(), abstraction, buckets, &evaluator, 1U);
    texas::solver::multiway::MultiwayRootBatchRunner runner(traversal, session.coordinator(), 1U, 8U);
...
static_assert(!std::is_copy_constructible_v<texas::solver::multiway::MultiwaySearchSession>);
static_assert(!std::is_copy_assignable_v<texas::solver::multiway::MultiwaySearchSession>);
static_assert(!std::is_move_constructible_v<texas::solver::multiway::MultiwaySearchSession>);
static_assert(!std::is_move_assignable_v<texas::solver::multiway::MultiwaySearchSession>);
...
TEST_CASE(multiway_search_session_request_rejects_stale_schema_v2_public_descriptors) {
    ...
    root = make_root(texas::core::Street::Flop);
...
TEST_CASE(multiway_search_session_initializes_request_local_postflop_state) {
    ...
    texas::solver::multiway::MultiwaySearchSession session(request, {&buckets}, 23U);
    ...
    EXPECT_EQ(first.metadata().source, texas::solver::multiway::MultiwayRangeBeliefSource::Supplied);
    EXPECT_EQ(second.metadata().source, texas::solver::multiway::MultiwayRangeBeliefSource::Supplied);
...
TEST_CASE(multiway_search_session_plans_important_deviation_without_mutating_current_root) {
    ...
    const auto request = make_request(root);
    texas::solver::multiway::MultiwaySearchSession session(request, {&buckets}, 23U);
    texas::solver::multiway::MultiwayActionAbstractionConfig action_config;
    action_config.translation_max_pseudo_harmonic_distance_basis_points = 1U;
    const texas::solver::multiway::MultiwayActionAbstraction abstraction(action_config);
    texas::solver::multiway::MultiwayDeviationExpansionConfig expansion;
    expansion.minimum_pseudo_harmonic_distance_basis_points = 1U;
    const auto planned = session.plan_local_expansion(
        abstraction, texas::games::multiway::MultiwayAction::Bet, 500, expansion);

    EXPECT_TRUE(planned.root.public_state.id != session.root_metadata().public_state);
    EXPECT_TRUE(planned.root.public_state.legal_actions.size() <= texas::solver::multiway::MULTIWAY_MAX_ABSTRACTED_ACTIONS);
...
TEST_CASE(multiway_search_session_isolates_request_data_and_belief_updates) {
    ...
    const auto buckets = make_buckets(root);
    const auto request = make_request(root);
    const auto other_request = make_request(make_root(texas::core::Street::Flop));
    texas::solver::multiway::MultiwaySearchSession session(request, {&buckets}, 1U);
    texas::solver::multiway::MultiwaySearchSession other_session(other_request, {&buckets}, 1U);
    ...
    texas::solver::multiway::MultiwayBucketActionPolicy policy;
    const auto& table = buckets.table(texas::core::Street::Flop, request.root().public_state.board);
    ...
    const texas::solver::multiway::MultiwayRangeBeliefObservation observation(
        table, policy, texas::solver::multiway::MultiwayRangeBeliefSource::Blueprint, 0U, 7U);

    EXPECT_EQ(
        session.apply_observation(0, observation),
        texas::solver::multiway::MultiwayRangeBeliefUpdateResult::Applied);
    EXPECT_EQ(session.belief(0).metadata().source, texas::solver::multiway::MultiwayRangeBeliefSource::Blueprint);
    EXPECT_EQ(session.belief(0).metadata().last_update_revision, 2U);
    EXPECT_EQ(session.belief(1).metadata().source, texas::solver::multiway::MultiwayRangeBeliefSource::Supplied);
    ...
    EXPECT_EQ(other_session.belief(0).metadata().source, texas::solver::multiway::MultiwayRangeBeliefSource::Supplied);
...
TEST_CASE(multiway_search_session_rejects_invalid_postflop_dependencies) {
    ...
    EXPECT_THROW(texas::solver::multiway::MultiwaySearchSession(request, {nullptr}, 1U), std::invalid_argument);
    EXPECT_THROW(texas::solver::multiway::MultiwaySearchSession(request, {&buckets}, 0U), std::invalid_argument);

    auto unavailable_root = root;
    unavailable_root.root_bucket = 1U;
    const auto unavailable_request = make_request(unavailable_root);
    EXPECT_THROW(
        texas::solver::multiway::MultiwaySearchSession(unavailable_request, {&buckets}, 1U),
        std::invalid_argument);
}

TEST_CASE(multiway_search_session_permits_preflop_without_bucket_registry) {
    const auto root = make_root(texas::core::Street::Preflop);
    const auto request = make_request(root);

    texas::solver::multiway::MultiwaySearchSession session(request, {nullptr}, 5U);
...
TEST_CASE(multiway_search_session_exposes_only_its_own_clean_row_snapshot) {
    ...
    const auto buckets = make_buckets(root);
    const auto request = make_request(root);
    texas::solver::multiway::MultiwaySearchSession session(request, {&buckets}, 11U);
...
TEST_CASE(multiway_search_session_rejects_invalid_clean_snapshot_metadata) {
    ...
    const auto buckets = make_buckets(root);
    const auto request = make_request(root);
    texas::solver::multiway::MultiwaySearchSession session(request, {&buckets}, 3U);
...
TEST_CASE(multiway_coordinator_checkpoint_restores_canonical_sparse_rows) {
    ...
    const auto buckets = make_buckets(root);
    const auto request = make_request(root);
    texas::solver::multiway::MultiwaySearchSession original(request, {&buckets}, 1U);
    ...
    texas::solver::multiway::MultiwayWorkerDeltaStream stream(0U, action_count);
    ...
    original.coordinator().merge_worker_streams(std::vector<texas::solver::multiway::MultiwayWorkerDeltaStream>{stream});
    ...
    texas::solver::multiway::MultiwaySearchSession resumed(request, {&buckets}, 1U);
...
TEST_CASE(multiway_blueprint_training_checkpoint_resumes_sparse_state) {
    ...
    const auto buckets = make_buckets(root);
    const auto request = make_request(root);
    texas::solver::multiway::MultiwaySolverCoordinator original_coordinator(request);
    const texas::solver::multiway::MultiwayActionAbstraction abstraction;
    const texas::solver::multiway::MultiwayLeafEvaluator evaluator = {session_leaf, nullptr};
    texas::solver::multiway::MultiwayRootExternalSamplingTraversal original_traversal(
        original_coordinator, request.root(), abstraction, buckets, &evaluator, 1U);
    texas::solver::multiway::MultiwayRootBatchRunner original_runner(original_traversal, original_coordinator, 1U, 8U);
    texas::solver::multiway::MultiwayBlueprintConfig config;
    config.player_count = 2U;
    const auto identity = texas::solver::multiway::make_multiway_model_identity(config);
    texas::solver::multiway::MultiwayBlueprintTrainer original(identity, original_runner, original_coordinator, {}, 0x77U);
    original.run_batches(1U, 1U, 0x77U);
    const auto checkpoint = original.checkpoint();
    const auto expected = original.export_full_policy();

    texas::solver::multiway::MultiwaySolverCoordinator resumed_coordinator(request);
    texas::solver::multiway::MultiwayRootExternalSamplingTraversal resumed_traversal(
        resumed_coordinator, request.root(), abstraction, buckets, &evaluator, 1U);
    texas::solver::multiway::MultiwayRootBatchRunner resumed_runner(resumed_traversal, resumed_coordinator, 1U, 8U);
    texas::solver::multiway::MultiwayBlueprintTrainer resumed(identity, resumed_runner, resumed_coordinator, {}, 0x77U);
...
TEST_CASE(multiway_search_session_exports_and_freezes_only_actual_hero_hand_policy) {
    ...
    const auto buckets = make_buckets(root);
    const auto request = make_request(root);
    texas::solver::multiway::MultiwaySearchSession session(request, {&buckets}, 19U);
...
TEST_CASE(multiway_search_session_carries_posteriors_into_reroots) {
    ...
    const auto next = session.make_next_round_root(make_root(texas::core::Street::Turn));
    EXPECT_EQ(next.public_state.betting.street, texas::core::Street::Turn);
    ...
    const auto reroot = session.make_reroot_root(make_root(texas::core::Street::Flop));
    EXPECT_EQ(reroot.public_state.betting.street, texas::core::Street::Flop);
    EXPECT_THROW(session.make_next_round_root(make_root(texas::core::Street::Flop)), std::invalid_argument);
}
...
TEST_CASE(multiway_decision_session_replaces_round_state_on_reroot) {
    ...
    const auto buckets = make_buckets(root);
    const auto request = make_request(root);
    texas::solver::multiway::MultiwayDecisionSession runtime(request, {&buckets});
    const auto actual = combo_id(root.private_ranges.ranges[0][0].hole);
    ...
    runtime.reroot(make_root(texas::core::Street::Flop), request.cfr_config(), request.limits(), false);
    EXPECT_EQ(runtime.root_revision(), 2U);
    EXPECT_EQ(runtime.round().root_metadata().revision, 2U);
    EXPECT_TRUE(!runtime.round().export_hero_policy(0, actual).actual_hand_frozen);
    EXPECT_EQ(runtime.round().root_metadata().public_state, make_root(texas::core::Street::Flop).public_state.id);
}
...
TEST_CASE(multiway_search_session_public_types_are_directly_available) {
    const auto root = make_root(texas::core::Street::Preflop);
    const texas::solver::multiway::MultiwaySolveRequest request = make_request(root);
    const texas::solver::multiway::MultiwaySearchSessionDependencies dependencies = {nullptr};
    texas::solver::multiway::MultiwaySearchSession session(request, dependencies, 3U);

    const texas::solver::multiway::MultiwaySearchSessionRootMetadata metadata = session.root_metadata();
    const texas::solver::multiway::MultiwaySearchSessionRowView rows = session.row_view();
...
TEST_CASE(multiway_search_session_owns_continuation_selector_dependency) {
    const auto root = make_root(texas::core::Street::Preflop);
    const auto selector = std::make_shared<const texas::solver::multiway::MultiwayFixedContinuationSelector>(
        texas::solver::multiway::MultiwayContinuationPolicyKind::Blueprint);
    const texas::solver::multiway::MultiwaySearchSessionDependencies dependencies{nullptr, selector};
    texas::solver::multiway::MultiwaySearchSession session(make_request(root), dependencies, 1U);
