...
texas::games::multiway::MultiwayState three_handed(int stack = 1000) {
    texas::games::multiway::MultiwayGameConfig config;
    ...
    config.street = texas::core::Street::Flop;
    return texas::games::multiway::MultiwayState::initial(config);
}

bool has(const std::vector<texas::games::multiway::MultiwayAction>& actions, texas::games::multiway::MultiwayAction action) {
...
TEST_CASE(multiway_rejects_invalid_seat_configuration) {
    ...
    EXPECT_THROW(texas::games::multiway::MultiwayState::initial(config), std::invalid_argument);
...
TEST_CASE(multiway_initializes_variable_seats_and_action_ring) {
    ...
    EXPECT_TRUE(has(state.legal_actions(), texas::games::multiway::MultiwayAction::Check));
    EXPECT_TRUE(has(state.legal_actions(), texas::games::multiway::MultiwayAction::Bet));
    EXPECT_TRUE(has(state.legal_actions(), texas::games::multiway::MultiwayAction::AllIn));
}
...
TEST_CASE(multiway_initial_contributions_reduce_remaining_stacks) {
    ...
    config.initial_street_contributions = {0, 0, 0};
    const auto state = texas::games::multiway::MultiwayState::initial(config);
...
TEST_CASE(multiway_six_seat_ring_wraps_without_two_player_assumptions) {
    ...
    auto state = texas::games::multiway::MultiwayState::initial(config);
    EXPECT_EQ(state.current_player(), 4);
    state = state.apply(texas::games::multiway::MultiwayAction::Check);
...
TEST_CASE(multiway_checks_walk_the_ring_and_complete_the_street) {
    const auto final = three_handed().apply(texas::games::multiway::MultiwayAction::Check)
                                     .apply(texas::games::multiway::MultiwayAction::Check)
                                     .apply(texas::games::multiway::MultiwayAction::Check);
...
TEST_CASE(multiway_full_bet_reopens_every_other_live_seat) {
    const auto bet = three_handed().apply(texas::games::multiway::MultiwayAction::Bet, 200);
    ...
    EXPECT_TRUE(has(bet.legal_actions(), texas::games::multiway::MultiwayAction::Fold));
    EXPECT_TRUE(has(bet.legal_actions(), texas::games::multiway::MultiwayAction::Call));
    EXPECT_TRUE(has(bet.legal_actions(), texas::games::multiway::MultiwayAction::Raise));
}

TEST_CASE(multiway_full_raise_returns_action_to_prior_callers) {
    const auto state = three_handed().apply(texas::games::multiway::MultiwayAction::Bet, 100)
                                     .apply(texas::games::multiway::MultiwayAction::Call)
                                     .apply(texas::games::multiway::MultiwayAction::Raise, 300);
...
TEST_CASE(multiway_short_all_in_does_not_reopen_prior_caller) {
    ...
    config.first_player = 0;
    auto state = texas::games::multiway::MultiwayState::initial(config)
                     .apply(texas::games::multiway::MultiwayAction::Bet, 100)
                     .apply(texas::games::multiway::MultiwayAction::Call)
                     .apply(texas::games::multiway::MultiwayAction::AllIn);
    EXPECT_EQ(state.current_bet(), 150);
    EXPECT_EQ(state.current_player(), 0);
    EXPECT_TRUE(!state.may_raise()[0]);
    EXPECT_TRUE(!has(state.legal_actions(), texas::games::multiway::MultiwayAction::Raise));
    EXPECT_TRUE(has(state.legal_actions(), texas::games::multiway::MultiwayAction::Call));
}
...
TEST_CASE(multiway_snapshot_round_trips_an_arbitrary_live_subgame_root) {
    const auto live = three_handed()
                          .apply(texas::games::multiway::MultiwayAction::Bet, 100)
                          .apply(texas::games::multiway::MultiwayAction::Call);
    const auto restored = texas::games::multiway::MultiwayState::from_snapshot(live.snapshot());
...
TEST_CASE(multiway_snapshot_rejects_inconsistent_raise_or_turn_metadata) {
    ...
    EXPECT_THROW(texas::games::multiway::MultiwayState::from_snapshot(snapshot), std::invalid_argument);
...
TEST_CASE(multiway_fold_terminates_when_one_player_remains) {
    const auto state = three_handed().apply(texas::games::multiway::MultiwayAction::Bet, 100)
                                     .apply(texas::games::multiway::MultiwayAction::Fold)
                                     .apply(texas::games::multiway::MultiwayAction::Fold);
...
TEST_CASE(multiway_covering_stack_has_no_betting_actions_after_all_in_calls) {
    ...
    EXPECT_EQ(state.next_node_kind(), texas::games::multiway::MultiwayNextNodeKind::BoardRunout);
}
...
TEST_CASE(multiway_snapshot_rejects_twenty_fully_matched_acted_pending_seats) {
    for (std::size_t seats = 2; seats <= 6; ++seats) {
        for (std::size_t variant = 0; variant < 4; ++variant) {
            ...
            snapshot.current_player = static_cast<texas::core::PlayerId>(offender);
            snapshot.pending[offender] = true;

            EXPECT_THROW(
                texas::games::multiway::MultiwayState::from_snapshot(snapshot),
...
TEST_CASE(multiway_snapshot_accepts_twenty_acted_pending_seats_below_the_current_bet) {
    for (std::size_t seats = 2; seats <= 6; ++seats) {
        for (int faced : {0, 25, 50, 75}) {
            ...
            snapshot.current_bet = 100;
            snapshot.last_full_raise_size = 100;
            snapshot.current_player = static_cast<texas::core::PlayerId>(responder);
            snapshot.last_aggressor = static_cast<texas::core::PlayerId>(aggressor);

            const auto restored = texas::games::multiway::MultiwayState::from_snapshot(snapshot);
...
TEST_CASE(multiway_lone_final_responder_can_only_fold_or_call) {
    for (const int covering_stack : {101, 102, 103, 104, 105, 110, 120, 125, 150, 175,
        ...
        config.initial_contributions = {0, 0, 0};
        config.initial_street_contributions = {0, 0, 0};
        const auto responder = texas::games::multiway::MultiwayState::initial(config)
            .apply(texas::games::multiway::MultiwayAction::AllIn)
            .apply(texas::games::multiway::MultiwayAction::Call);
        EXPECT_EQ(responder.legal_actions(), (std::vector<texas::games::multiway::MultiwayAction>{
            texas::games::multiway::MultiwayAction::Fold, texas::games::multiway::MultiwayAction::Call}));
    }
}
...
TEST_CASE(multiway_all_in_has_one_canonical_transition) {
    ...
    config.initial_street_contributions = {0, 0, 0};
    config.first_player = 0;
    const auto facing_bet = texas::games::multiway::MultiwayState::initial(config)
                                .apply(texas::games::multiway::MultiwayAction::Bet, 100);
    EXPECT_TRUE(!has(facing_bet.legal_actions(), texas::games::multiway::MultiwayAction::Raise));
    EXPECT_TRUE(has(facing_bet.legal_actions(), texas::games::multiway::MultiwayAction::AllIn));
    EXPECT_THROW(facing_bet.apply(texas::games::multiway::MultiwayAction::Raise, 150), std::invalid_argument);
}
...
TEST_CASE(multiway_rejects_invalid_street_transitions) {
    const auto complete = three_handed().apply(texas::games::multiway::MultiwayAction::Check)
                                        .apply(texas::games::multiway::MultiwayAction::Check)
                                        .apply(texas::games::multiway::MultiwayAction::Check);
    EXPECT_THROW(complete.begin_next_street(texas::core::Street::River, 0), std::invalid_argument);
    EXPECT_THROW(complete.begin_next_street(texas::core::Street::Turn, 3), std::invalid_argument);
}
...
TEST_CASE(multiway_next_street_resets_betting_and_uses_requested_ring_start) {
    const auto complete = three_handed().apply(texas::games::multiway::MultiwayAction::Check)
                                        .apply(texas::games::multiway::MultiwayAction::Check)
                                        .apply(texas::games::multiway::MultiwayAction::Check);
    const auto turn = complete.begin_next_street(texas::core::Street::Turn, 2);
    EXPECT_EQ(turn.street(), texas::core::Street::Turn);
...
TEST_CASE(multiway_river_completion_is_terminal_without_pot_evaluation) {
    auto state = three_handed().apply(texas::games::multiway::MultiwayAction::Check)
                               .apply(texas::games::multiway::MultiwayAction::Check)
                               .apply(texas::games::multiway::MultiwayAction::Check)
                               .begin_next_street(texas::core::Street::Turn, 0)
                               .apply(texas::games::multiway::MultiwayAction::Check)
                               .apply(texas::games::multiway::MultiwayAction::Check)
                               .apply(texas::games::multiway::MultiwayAction::Check)
                               .begin_next_street(texas::core::Street::River, 0)
                               .apply(texas::games::multiway::MultiwayAction::Check)
                               .apply(texas::games::multiway::MultiwayAction::Check)
                               .apply(texas::games::multiway::MultiwayAction::Check);
    EXPECT_TRUE(state.is_terminal());
...
TEST_CASE(multiway_rejects_illegal_actions_and_bad_raise_sizes) {
    const auto state = three_handed();
    EXPECT_THROW(state.apply(texas::games::multiway::MultiwayAction::Call), std::invalid_argument);
    EXPECT_THROW(state.apply(texas::games::multiway::MultiwayAction::Bet, 50), std::invalid_argument);
    const auto bet = state.apply(texas::games::multiway::MultiwayAction::Bet, 100);
    EXPECT_THROW(bet.apply(texas::games::multiway::MultiwayAction::Raise, 150), std::invalid_argument);
    EXPECT_THROW(bet.begin_next_street(texas::core::Street::Turn, 0), std::logic_error);
}
