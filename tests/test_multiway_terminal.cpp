...
texas::Strength hand(std::uint64_t value) {
    return texas::Strength{value};
}
...
texas::MultiwayTerminalInput input(
    ...
    texas::MultiwayTerminalInput value;
...
TEST_CASE(multiway_terminal_rejects_mismatched_or_invalid_inputs) {
    EXPECT_THROW(texas::settle_multiway_terminal(input({100, 100}, {false}, {1, 2})), std::invalid_argument);
    EXPECT_THROW(texas::settle_multiway_terminal(input({100, -1}, {false, false}, {1, 2})), std::invalid_argument);
    EXPECT_THROW(texas::settle_multiway_terminal(input({100, 100}, {true, true}, {1, 2})), std::invalid_argument);
}

TEST_CASE(multiway_terminal_single_uncontested_pot_awards_all_contributions) {
    const auto result = texas::settle_multiway_terminal(input({100, 100, 100}, {false, true, true}, {1, 99, 99}));
...
TEST_CASE(multiway_terminal_equal_contributions_make_one_main_pot) {
    const auto result = texas::settle_multiway_terminal(input({200, 200, 200}, {false, false, false}, {3, 8, 5}));
    EXPECT_EQ(result.pots.size(), std::size_t{1});
...
TEST_CASE(multiway_terminal_builds_main_and_side_pot_for_one_short_stack) {
    const auto result = texas::settle_multiway_terminal(input({100, 300, 300}, {false, false, false}, {9, 5, 7}));
...
TEST_CASE(multiway_terminal_builds_three_ordered_pots_for_staggered_all_ins) {
    const auto result = texas::settle_multiway_terminal(input({100, 200, 400, 400}, {false, false, false, false}, {9, 8, 7, 6}));
...
TEST_CASE(multiway_terminal_folded_player_contributes_but_cannot_win) {
    const auto result = texas::settle_multiway_terminal(input({200, 200, 200}, {false, true, false}, {5, 99, 8}));
...
TEST_CASE(multiway_terminal_refunds_uncalled_excess_bet) {
    const auto result = texas::settle_multiway_terminal(input({300, 100, 100}, {false, true, true}, {1, 2, 3}));
    EXPECT_EQ(result.pots.size(), std::size_t{1});
    EXPECT_EQ(result.pots[0].amount, 300);
...
TEST_CASE(multiway_terminal_refunds_excess_after_short_all_in) {
    const auto result = texas::settle_multiway_terminal(input({500, 200, 200}, {false, false, false}, {5, 8, 7}));
    EXPECT_EQ(result.pots.size(), std::size_t{1});
    EXPECT_EQ(result.pots[0].amount, 600);
...
TEST_CASE(multiway_terminal_tied_main_pot_splits_evenly) {
    const auto result = texas::settle_multiway_terminal(input({150, 150, 150, 150}, {false, false, false, false}, {8, 8, 3, 1}));
    EXPECT_EQ(result.payouts[0], 300);
...
TEST_CASE(multiway_terminal_tied_odd_chip_uses_explicit_positional_order) {
    ...
    const auto result = texas::settle_multiway_terminal(terminal_input);
...
TEST_CASE(multiway_terminal_different_winners_can_take_different_side_pots) {
    const auto result = texas::settle_multiway_terminal(input({100, 300, 300}, {false, false, false}, {10, 5, 8}));
    EXPECT_EQ(result.payouts[0], 300);
    EXPECT_EQ(result.payouts[2], 400);
    EXPECT_EQ(result.utilities[0], 200.0);
...
TEST_CASE(multiway_terminal_tie_can_be_limited_to_the_main_pot) {
    const auto result = texas::settle_multiway_terminal(input({100, 300, 300}, {false, false, false}, {9, 9, 7}));
...
TEST_CASE(multiway_terminal_refunds_are_not_side_pots) {
    const auto result = texas::settle_multiway_terminal(input({400, 100, 0}, {false, false, true}, {8, 7, 1}));
    EXPECT_EQ(result.pots.size(), std::size_t{1});
...
TEST_CASE(multiway_terminal_utilities_are_zero_sum_with_refunds) {
    const auto result = texas::settle_multiway_terminal(input({100, 250, 400, 400}, {false, false, false, false}, {4, 10, 8, 6}));
...
TEST_CASE(multiway_terminal_rake_honors_no_flop_no_drop_and_explicit_zero) {
    ...
    const auto no_drop = texas::settle_multiway_terminal(terminal_input);
    EXPECT_EQ(no_drop.rake_taken, 0);
    EXPECT_EQ(sum(no_drop.payouts), 300);

    terminal_input.rake_policy = texas::MultiwayRakePolicy::explicit_zero();
    terminal_input.flop_seen = true;
    const auto explicit_zero = texas::settle_multiway_terminal(terminal_input);
...
TEST_CASE(multiway_terminal_handles_zero_contribution_folded_seat) {
    const auto result = texas::settle_multiway_terminal(input({100, 100, 0}, {false, false, true}, {4, 8, 99}));
    EXPECT_EQ(result.pots.size(), std::size_t{1});
...
TEST_CASE(multiway_terminal_can_settle_a_precomputed_pot_layout) {
    ...
    const auto layout = texas::build_multiway_pot_layout(terminal_input.contributions, terminal_input.folded);
    const auto result = texas::settle_multiway_terminal(terminal_input, layout);
    EXPECT_EQ(result.pots.size(), std::size_t{2});
    EXPECT_EQ(result.payouts[0], 300);
    EXPECT_EQ(result.payouts[2], 400);
}
...
TEST_CASE(multiway_terminal_rejects_nonconserving_precomputed_layout) {
    ...
    texas::MultiwayPotLayout layout;
    layout.refunds = {0, 0};
    layout.pots.push_back({100, 100, {0, 1}});
    EXPECT_THROW(texas::settle_multiway_terminal(terminal_input, layout), std::invalid_argument);
}
