...
const std::vector<double>* find_strategy(const texas::SolveOutput& output, const std::string& key) {
...
TEST_CASE(simd_discount_regrets_vector_matches_scalar) {
    ...
    texas::discount_regrets(via_dispatch.data(), via_dispatch.size(), 0.51, 0.27);
    texas::discount_regrets_scalar(via_scalar.data(), via_scalar.size(), 0.51, 0.27);
...
TEST_CASE(simd_discount_regrets_preserves_special_lanes_across_forty_shapes) {
    for (std::size_t length = 1; length <= 40; ++length) {
        ...
        auto via_dispatch = base;
        auto via_scalar = base;
        texas::discount_regrets(
            via_dispatch.data() + offset, length, 0.51, 0.27);
        texas::discount_regrets_scalar(
...
TEST_CASE(simd_discount_strategy_sum_vector_matches_scalar) {
    ...
    auto via_dispatch = base;
    auto via_scalar = base;
    texas::discount_strategy_sum(via_dispatch.data(), via_dispatch.size(), 0.875);
    texas::discount_strategy_sum_scalar(via_scalar.data(), via_scalar.size(), 0.875);
...
TEST_CASE(simd_compute_strategy_row_matches_scalar) {
    ...
    texas::compute_strategy_row(regrets.data(), via_dispatch.data(), regrets.size());
    texas::compute_strategy_row_scalar(regrets.data(), via_scalar.data(), regrets.size());
...
TEST_CASE(simd_copy_values_matches_scalar_and_handles_empty_rows) {
    ...
    texas::copy_values(empty_out.data(), empty.data(), empty.size());

    const std::vector<double> values = {1.0, -2.5, 3.25, 4.75, -9.0};
    std::vector<double> copied(values.size(), 0.0);
    texas::copy_values(copied.data(), values.data(), values.size());
...
TEST_CASE(simd_dot_product_matches_scalar_expectations) {
    ...
    const auto via_dispatch = texas::dot_product(lhs.data(), rhs.data(), lhs.size());
    const auto via_scalar = texas::dot_product_scalar(lhs.data(), rhs.data(), lhs.size());

    EXPECT_NEAR(via_dispatch, via_scalar, TOL);
    EXPECT_NEAR(via_dispatch, 11.0, TOL);
    EXPECT_EQ(texas::dot_product(lhs.data(), rhs.data(), 0), 0.0);
}
...
TEST_CASE(simd_dot_product_strided_matches_scalar_expectations) {
    ...
    const auto via_dispatch = texas::dot_product_strided(lhs.data(), rhs.data(), lhs.size(), 2);
    const auto via_scalar = texas::dot_product_strided_scalar(lhs.data(), rhs.data(), lhs.size(), 2);

    EXPECT_NEAR(via_dispatch, via_scalar, TOL);
    EXPECT_NEAR(via_dispatch, -5.5, TOL);
    EXPECT_EQ(texas::dot_product_strided(lhs.data(), rhs.data(), 0, 2), 0.0);
}

TEST_CASE(simd_row_normalization_matches_scalar_and_handles_zero_length) {
    std::vector<double> empty_in;
    std::vector<double> empty_out;
    texas::normalize_row(empty_in.data(), empty_out.data(), empty_in.size());
    ...
    texas::normalize_row(values.data(), via_dispatch.data(), values.size());
    texas::normalize_row(values.data(), via_scalar.data(), values.size());
...
TEST_CASE(simd_row_reduction_helpers_match_scalar_and_cover_special_values) {
    ...
    EXPECT_TRUE(texas::reduce_action_values(values.data(), 0) == 0.0);
    EXPECT_TRUE(texas::reduce_weighted_action_values(values.data(), weights.data(), 0) == 0.0);

    EXPECT_TRUE(texas::reduce_action_values(values.data(), 2) == values[0] + values[1]);
    EXPECT_TRUE(texas::reduce_weighted_action_values(values.data(), weights.data(), 2) ==
    ...
    const double total = texas::positive_regrets_and_total(regrets.data(), positive.data(), regrets.size());
...
TEST_CASE(simd_update_strategy_sum_matches_scalar_and_handles_small_rows) {
    std::vector<double> empty;
    texas::update_strategy_sum(empty.data(), empty.data(), 0, 1.0);
    ...
    texas::update_strategy_sum(via_dispatch.data(), strategy.data(), strategy.size(), 2.5);
    texas::update_strategy_sum_scalar(via_scalar.data(), strategy.data(), strategy.size(), 2.5);
...
TEST_CASE(simd_update_regret_sum_matches_scalar) {
    ...
    texas::update_regret_sum(via_dispatch.data(), action_values.data(), action_values.size(), 1.75, 0.5);
    texas::update_regret_sum_scalar(via_scalar.data(), action_values.data(), action_values.size(), 1.75, 0.5);
...
TEST_CASE(simd_kuhn_solve_is_deterministic_under_current_backend) {
    const auto first = texas::solve_kuhn(1000, 1.5, 0.0, 2.0);
    const auto second = texas::solve_kuhn(1000, 1.5, 0.0, 2.0);
