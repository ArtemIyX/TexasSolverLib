...
core::MultiwayLeafEvaluationRequest make_cacheable_request(std::uint64_t public_state = 11U) {
    core::MultiwayLeafEvaluationRequest request;
    request.traverser = 0;
...
TEST_CASE(multiway_rollout_leaf_exact_all_in_is_deterministic_for_all_profiles) {
    ...
    EXPECT_EQ(first.runout_mode, core::MultiwayRolloutRunoutMode::Exact);
...
TEST_CASE(multiway_rollout_leaf_uses_finite_seeded_fallback_when_exact_runouts_are_capped) {
    ...
    EXPECT_EQ(result.runout_mode, core::MultiwayRolloutRunoutMode::Seeded);
...
TEST_CASE(multiway_rollout_leaf_cache_reuses_same_seed_context_and_records_diagnostics) {
    RolloutFixture fixture;
    auto context = make_context(fixture);
    core::MultiwayContinuationCache cache(2U);
    core::MultiwayContinuationDiagnostics diagnostics;
    context.cache = &cache;
    context.diagnostics = &diagnostics;
    context.selected_policy = core::MultiwayContinuationPolicyKind::CallBiased;
    ...
    EXPECT_EQ(diagnostics.policy_mode, core::MultiwayContinuationPolicyKind::Blueprint);
    EXPECT_EQ(diagnostics.runout_mode, core::MultiwayRolloutRunoutMode::Exact);
...
TEST_CASE(multiway_rollout_leaf_reports_different_seed_pair_variance) {
    ...
    core::MultiwayContinuationCache cache(4U);
    core::MultiwayContinuationDiagnostics diagnostics;
    context.cache = &cache;
    context.diagnostics = &diagnostics;
    const auto request = make_cacheable_request();
    ...
    EXPECT_EQ(diagnostics.runout_mode, core::MultiwayRolloutRunoutMode::Seeded);
    for (std::size_t policy = 0; policy < first.values.size(); ++policy) {
...
TEST_CASE(multiway_rollout_leaf_cache_separates_range_and_private_contexts) {
    RolloutFixture fixture;
    auto context = make_context(fixture);
    core::MultiwayContinuationCache cache(3U);
    core::MultiwayContinuationDiagnostics diagnostics;
    context.cache = &cache;
    context.diagnostics = &diagnostics;
    core::MultiwayRolloutProfileResult result;
    ...
    EXPECT_TRUE(core::evaluate_multiway_rollout_profiles(request, context, &result));
...
TEST_CASE(multiway_rollout_leaf_cache_stops_admission_at_byte_cap) {
    RolloutFixture fixture;
    auto context = make_context(fixture);
    core::MultiwayContinuationCache cache(
        4U, core::MultiwayContinuationCache::entry_bytes());
    core::MultiwayContinuationDiagnostics diagnostics;
    context.cache = &cache;
    context.diagnostics = &diagnostics;
    core::MultiwayRolloutProfileResult result;

    EXPECT_TRUE(core::evaluate_multiway_rollout_profiles(
    ...
    EXPECT_EQ(cache.size(), std::size_t{1});
    EXPECT_EQ(cache.memory_bytes(), core::MultiwayContinuationCache::entry_bytes());
    EXPECT_EQ(fixture.input_calls, std::size_t{3});
    EXPECT_EQ(diagnostics.cache_hits, std::uint64_t{0});
    EXPECT_EQ(diagnostics.cache_misses, std::uint64_t{3});
