...
const texas::preflop::PreflopBettingTree::Node& root_of(
    const texas::preflop::PreflopBettingTree& tree) {
    return tree.nodes.at(0);
}

bool has_action(
    const texas::preflop::PreflopBettingTree::Node& node,
...
TEST_CASE(preflop_tree_uses_single_configured_raise_size) {
    ...
    const auto tree = texas::preflop::PreflopBettingTree::build(config);
...
TEST_CASE(preflop_tree_root_raise_uses_raise_token) {
    const auto tree = texas::preflop::PreflopBettingTree::build(base_config());
...
TEST_CASE(preflop_tree_big_blind_check_ends_preflop) {
    ...
    EXPECT_EQ(
        ...
        texas::preflop::PreflopBettingTree::NodeKind::EquityLeaf);
}
...
TEST_CASE(preflop_tree_raise_fold_is_fold_leaf) {
    ...
    EXPECT_EQ(
        ...
        texas::preflop::PreflopBettingTree::NodeKind::Fold);
}
...
TEST_CASE(preflop_tree_decisions_never_have_empty_action_menus) {
    ...
    for (const auto& node : tree.nodes) {
        if (node.kind == texas::preflop::PreflopBettingTree::NodeKind::Decision) {
...
TEST_CASE(preflop_tree_rejects_non_preflop_config) {
    ...
    EXPECT_THROW(
        texas::preflop::PreflopBettingTree::build(config),
