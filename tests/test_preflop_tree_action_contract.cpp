...
const core::PreflopBettingTree::Node& root_of(
    const core::PreflopBettingTree& tree) {
    return tree.nodes.at(0);
}

bool has_action(
    const core::PreflopBettingTree::Node& node,
...
TEST_CASE(preflop_tree_uses_single_configured_raise_size) {
    ...
    const auto tree = core::PreflopBettingTree::build(config);
...
TEST_CASE(preflop_tree_root_raise_uses_raise_token) {
    const auto tree = core::PreflopBettingTree::build(base_config());
...
TEST_CASE(preflop_tree_big_blind_check_ends_preflop) {
    ...
    EXPECT_EQ(
        ...
        core::PreflopBettingTree::NodeKind::EquityLeaf);
}
...
TEST_CASE(preflop_tree_raise_fold_is_fold_leaf) {
    ...
    EXPECT_EQ(
        ...
        core::PreflopBettingTree::NodeKind::Fold);
}
...
TEST_CASE(preflop_tree_decisions_never_have_empty_action_menus) {
    ...
    for (const auto& node : tree.nodes) {
        if (node.kind == core::PreflopBettingTree::NodeKind::Decision) {
...
TEST_CASE(preflop_tree_rejects_non_preflop_config) {
    ...
    EXPECT_THROW(
        core::PreflopBettingTree::build(config),
...
TEST_CASE(preflop_tree_rejects_resolved_private_cards) {
    ...
    config.initial_hole_cards =
        std::array<std::array<std::uint8_t, 2>, 2>{{
            ...
            {core::card_to_int(13, 2), core::card_to_int(13, 3)},
        }};
    EXPECT_THROW(
        core::PreflopBettingTree::build(config),
        std::invalid_argument);
